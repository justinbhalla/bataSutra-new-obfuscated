...
function setYear(){var y=(new Date).getFullYear().toString(),a=document.getElementById("year");if(a)a.textContent=y;}
...
/* ===== Boot ===== */
function hydrate(){getHeadlines().then(function(d){paint(d);}).then(function(){setTimeout(function(){getHeadlines().then(paint);},2000);});}
function boot(){
  setYear();reduceMotion();hydrate();
  /* Refresh every 5 minutes and on tab focus */
  setInterval(hydrate,300000);
  document.addEventListener("visibilitychange",function(){if(!document.hidden)hydrate()});
  /* Simple Hall of Fame search */
  var q=document.getElementById("q"); if(q){q.addEventListener("input",function(){var val=(q.value||"").toLowerCase();var rows=document.querySelectorAll("#hofBody tr");for(var i=0;i<rows.length;i++){var tr=rows[i];var text=(tr.textContent||"").toLowerCase();tr.style.display=text.indexOf(val)>-1?"": "none";}});}
}
